  ...
  return (
    <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
      ...
      <div className="grid grid-cols-25 gap-0 border border-gray-300 rounded-md overflow-hidden">
        <div className="text-xs font-semibold border-r border-b border-gray-300 p-1">
          State
        </div>
        {[...Array(24)].map((_, i) => (
          <div
            key={i}
            className="text-xs text-center border-r border-b border-gray-300 p-1"
          >
            {i}
          </div>
        ))}
        {schedule.map((segment, index) => (
          <React.Fragment key={index}>
            <div className="text-xs border-r border-b border-gray-300 p-1">
              {segment.type}
            </div>
            <div
              className={`${stateColorMap[segment.type]} h-4`}
              style={{
                gridColumn: `span ${Math.round(segment.hours * (24 / totalHours))}`,
              }}
            />
          </React.Fragment>
        ))}
