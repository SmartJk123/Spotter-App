...
class HOSCalculator:
    ...
    def add_driving_time(self, hours):
        ...
        hours_to_drive = min(hours, driving_hours_left, on_duty_hours_left, cycle_hours_left)
        ...
        if hours_to_drive > 0:
            ...
            self.daily_log.append({"type": "Driving", "hours": round(hours_to_drive, 2)})
    ...
    def take_rest_break(self):
        ...
        if not self.is_rest_break_taken:
            self.daily_log.append({"type": "Off Duty", "hours": self.REST_BREAK_MIN_DURATION})
    ...
    def end_day_with_rest(self):
        ...
        total_daily_hours = self.current_driving_hours + self.current_on_duty_hours + self.current_off_duty_hours
        remaining_hours = 24.0 - total_daily_hours

        if remaining_hours > 0:
            self.daily_log.append({"type": "Sleeper Berth", "hours": round(remaining_hours, 2)})
            return True
        return False
